...
from faker.providers.person.es_ES import Provider as NameProvider

from cythonsim.simrandom cimport RandomPool
# from cythonsim.simrandom import RandomPool
...
VACCINE_TO_STR = {
    VaccineType.NONE: 'None',
    VaccineType.PFIZER_BIONTECH: 'Pfizer-BioNTech',
    VaccineType.MODERNA: 'Moderna',
    VaccineType.ASTRAZENECA: 'AstraZeneca',
    VaccineType.JANSSEN: 'Janssen',
    VaccineType.NOVAVAX: 'Novavax',
...
STATE_TO_STR = {
    ...
    PersonState.QUARANTINED: 'QUARANTINED',
    ...
    # NEW: Vaccination
    PersonState.IMMUNIZED: 'Immunized',
...
cdef struct Person:
    ...
    uint8 vaccine
    uint8 vaccine_doses
    int16 day_of_first_dose
    int16 day_of_immunization
...
cdef void person_init(Person *self, int32 idx, uint8 age, uint8 sex) nogil:
    self.idx = idx
    self.age = age
    self.sex = sex
    self.is_infected = 0
    ...
    self.is_immunized = 0
    self.vaccine = VaccineType.NONE
    self.vaccine_doses = 0
    self.day_of_first_dose = 0
    self.day_of_immunization = 0

    openmp.omp_init_lock(&self.lock)


first_names = list(NameProvider.first_names)
last_names = list(NameProvider.last_names)
...
cdef str person_str(Person *self, int today=-1):
    ...
    result = f'{name}: {self.age} years, infection on day {self.day_of_infection}{days_ago}, {STATE_TO_STR[self.state]}' \
             f', {SEVERITY_TO_STR[self.symptom_severity]}, days left {self.days_left}, {queued}detected ' \
             f'{self.was_detected}, .ax. contacts {self.max_contacts_per_day} (others infected ' \
             f'{self.other_people_infected}{infectees}) (included_in_totals {self.included_in_totals})\n'

    result += f'VACCINE: {VACCINE_TO_STR[self.vaccine]}, doses: {self.vaccine_doses}'

    return result


# A person gets infected by another person (source)
...
cdef bint person_expose(Person *self, Context context, Person *source) nogil:
    ...
    # NEW: Vaccination
    if self.is_infected or self.has_immunity or self.is_vaccinated:
        return False
...
# NEW: Vaccination
cdef void person_vaccinate(Person *self, Vaccine vaccine, int dose, Context context) nogil:
    cdef bint p_immunization = 0
    self.vaccine = vaccine.vaccine_type
    self.vaccine_doses = dose

    if dose == 1:
        p_immunization = context.random.chance(vaccine.efectiveness_after_first_dose)
        self.day_of_first_dose = context.day
    elif dose == 2:
        p_immunization = context.random.chance(vaccine.efectiveness_after_second_dose)

    if p_immunization:
        self.day_of_immunization = context.day + vaccine.days_to_immunization

    if not vaccine.two_doses and dose == 1 or vaccine.two_doses and dose == 2:
        self.is_vaccinated = 1
        context.pop.vaccinate(self)
        context.total_vaccinated += 1
...
cdef void person_recover_introduced(Person *self, Context context) nogil:
    self.state = PersonState.RECOVERED
    person_become_removed(self, context)
...
cdef void person_advance(Person *self, Context context) nogil:
    ...
    if self.state == PersonState.INCUBATION:
        ...
        if self.symptom_severity != SymptomSeverity.ASYMPTOMATIC:
            ...
            p_test = context.random.chance(1 / context.hc.test_delay_avg_days)
            if not self.was_detected and p_test:
                context.hc.seek_testing(self, context)

        self.day_of_illness += 1

        # with gil:
        #     print(f'day_of_illness: {self.day_of_illness}, chance test: {p_test}')
        if self.days_left > 0:
            self.days_left -= 1
        if self.days_left == 0:
...
cdef class Vaccine:
    cdef public uint8 id
    cdef public uint8 vaccine_type
    cdef public uint8 two_doses
    cdef public uint8 days_between_doses
    cdef public int16 days_to_immunization
    cdef public float efectiveness_after_first_dose
    cdef public float efectiveness_after_second_dose

    def __init__(self, vaccine_type, id, two_doses, days_between_doses, days_to_immunization, efectiveness_after_first_dose,
                 efectiveness_after_second_dose):
        self.id = id
        self.vaccine_type = vaccine_type
        self.two_doses = two_doses
        self.days_between_doses = days_between_doses
        self.days_to_immunization = days_to_immunization
        self.efectiveness_after_first_dose = efectiveness_after_first_dose
        self.efectiveness_after_second_dose = efectiveness_after_second_dose


cdef class VaccinationStage:
    cdef public Vaccine vaccine
    cdef public uint8 dose
    cdef public uint8 min_age
    cdef public uint8 max_age
    cdef public int16 rate
    cdef public int16 start_day
    cdef public int16 end_day

    def __init__(self, vaccine, dose, min_age, max_age, rate, start_day, end_day):
...
cdef class HealthcareSystem:
    ...
    cdef int32 test_delay_avg_days

    # NEW: Vaccination
    cdef dict vaccines
    cdef list vaccination_stages
    cdef list vaccination_queue
    cdef dict daily_vaccination_dose1
    cdef dict daily_vaccination_dose2
    cdef dict all_vaccination_dose1
    cdef dict all_vaccination_dose2
    ...
    def __init__(self, hospital_beds, icu_units, vaccines=(), vaccination_stages=()):
        ...
        self.test_delay_avg_days = 1
        self.p_detected_anyway = 0
        self.p_successful_tracing = 1.0

        # NEW: Vaccination
        self.vaccination_queue = []
        self.daily_vaccination_dose1 = {}
        self.daily_vaccination_dose2 = {}
        self.all_vaccination_dose1 = {}
        self.all_vaccination_dose2 = {}
        self._create_vaccines(vaccines)
        self._create_vaccination_stages(vaccination_stages)

        openmp.omp_init_lock(&self.lock)


    cdef _create_vaccines(self, vaccines):
        all_vaccines = {}
        for vaccine in vaccines:
            print(f"initializing vaccine: {vaccine}")
            params = {'id': vaccine['id'], 'two_doses': vaccine['two_doses'],
                      'days_between_doses': vaccine['days_between_doses'],
                      'days_to_immunization': vaccine['days_to_immunization'],
                      'efectiveness_after_first_dose': vaccine['efectiveness_after_first_dose'],
                      'efectiveness_after_second_dose': vaccine['efectiveness_after_second_dose']}
            if vaccine['name'] == 'Pfizer-BioNTech':
                v = Vaccine(VaccineType.PFIZER_BIONTECH, **params)
            elif vaccine['name'] == 'Moderna':
                v = Vaccine(VaccineType.MODERNA, **params)
            elif vaccine['name'] == 'AstraZeneca':
                v = Vaccine(VaccineType.ASTRAZENECA, **params)
            elif vaccine['name'] == 'Janssen':
                v = Vaccine(VaccineType.JANSSEN, **params)
            elif vaccine['name'] == 'Novavax':
                v = Vaccine(VaccineType.NOVAVAX, **params)
            else:
                v = Vaccine(VaccineType.NONE, **params)

            all_vaccines[vaccine['name']] = v
            self.daily_vaccination_dose1[vaccine['name']] = 0
            self.daily_vaccination_dose2[vaccine['name']] = 0
            self.all_vaccination_dose1[vaccine['name']] = 0
            self.all_vaccination_dose2[vaccine['name']] = 0

        self.vaccines = all_vaccines


    cdef _create_vaccination_stages(self, vaccination_stages):
        all_stages = []
        for stage in vaccination_stages:
            print(f"initializing vaccination stage: {stage}")
            vaccine = self.vaccines[stage['vaccine']]
            s = VaccinationStage(vaccine, stage['dose'], stage['min-age'], stage['max_age'],
                                       stage['rate'], stage['start_date'], stage['end_date'])
            all_stages.append(s)
        self.vaccination_stages = all_stages
    ...
    cdef iterate(self, Context context):
        ...
        # NEW: Vaccination
        cdef int total_people = context.pop.total_people
        cdef Person *people = context.pop.people
        ...
        for i in range(101 * 2):
            context.pop.vaccinated[i] = 0
            context.pop.immunized[i] = 0

        for vaccine in self.vaccines.values():
            self.daily_vaccination_dose1[VACCINE_TO_STR[vaccine.vaccine_type]] = 0
            self.daily_vaccination_dose2[VACCINE_TO_STR[vaccine.vaccine_type]] = 0

        for stage in self.vaccination_stages:
            if stage.start_day <= context.day <= stage.end_day and stage.rate > 0:
                if stage.dose == 1:
                    count = 0
                    start_idx = context.random.getint() % total_people
                    for i in range(total_people):
                        person_idx = (start_idx + i) % total_people
                        person = people + person_idx
                        if person.vaccine == VaccineType.NONE and person.vaccine_doses == 0 and \
                                person.state == PersonState.SUSCEPTIBLE:
                                # person.state not in (PersonState.IMMUNIZED, PersonState.HOSPITALIZED,
                                #                      PersonState.IN_ICU, PersonState.DEAD, PersonState.ILLNESS,
                                #                      PersonState.QUARANTINED, PersonState.INCUBATION):
    ...
    # RECOVERED
    # DEAD
                            if stage.min_age <= person.age <= stage.max_age:
                                person_vaccinate(person, stage.vaccine, stage.dose, context)
                                if stage.vaccine.two_doses:
                                    self.vaccination_queue.append(person_idx)
                                self.daily_vaccination_dose1[VACCINE_TO_STR[stage.vaccine.vaccine_type]] += 1
                                self.all_vaccination_dose1[VACCINE_TO_STR[stage.vaccine.vaccine_type]] += 1
                                count += 1
                                if count >= stage.rate:
                                    break
                elif stage.dose == 2:
                    count = 0
                    queue = self.vaccination_queue.copy()
                    for idx in queue:
                        person = context.pop.people + idx
                        if person.vaccine == stage.vaccine.vaccine_type and \
                                person.day_of_first_dose + stage.vaccine.days_between_doses <= context.day:
                            person_vaccinate(person, stage.vaccine, stage.dose, context)
                            self.vaccination_queue.remove(idx)
                            self.daily_vaccination_dose2[VACCINE_TO_STR[stage.vaccine.vaccine_type]] += 1
                            self.all_vaccination_dose2[VACCINE_TO_STR[stage.vaccine.vaccine_type]] += 1
                            count += 1
                            if count >= stage.rate:
                                break

                    # print(2, context.day, len(self.vaccination_queue), stage.rate)

        # print(self.daily_vaccination_dose1)
        # print(self.daily_vaccination_dose2)
...
cdef class Disease:
    # cdef list p_infections_list
    # cdef float p_infection, p_asymptomatic, p_hospital_death
    cdef float current_p_infection, p_asymptomatic, p_hospital_death
    ...
    def __init__(self,
        ...
        self.current_p_infection = p_infection[0][0]
    ...
    cdef iterate(self, Context context):
        self.current_p_infection = self.p_infection.get_greatest_lte(context.day)
        # print(context.day, self.current_p_infection)
    ...
    cdef float get_infectiousness_over_time(self, int day) nogil:
        return self.infectiousness_over_time.get(day, 0) * self.current_p_infection
...
cdef class Population:
    ...
    # NEW: Vaccination
    cdef int[::1] vaccinated, all_vaccinated, immunized, all_immunized
    ...
    def __init__(self, age_structure, contacts_per_day):
        self.nr_ages = age_structure.shape[0] // 2

        age_counts = np.empty(self.nr_ages * 2, dtype=np.int32)
        # for age, count in age_structure:
        #     age_counts[age] = count

        age_counts[:] = age_structure[:]
    ...
    def _init_stats(self, age_counts):
        ...
        self.infected = np.zeros(nr_ages * 2, dtype=np.int32)
        self.detected = np.zeros(nr_ages * 2, dtype=np.int32)
        self.all_detected = np.zeros(nr_ages * 2, dtype=np.int32)
        self.all_infected = np.zeros(nr_ages * 2, dtype=np.int32)
        self.recovered = np.zeros(nr_ages * 2, dtype=np.int32)
        self.hospitalized = np.zeros(nr_ages * 2, dtype=np.int32)
        self.in_icu = np.zeros(nr_ages * 2, dtype=np.int32)
        self.dead = np.zeros(nr_ages * 2, dtype=np.int32)

        # NEW: Vaccination
        self.vaccinated = np.zeros(nr_ages * 2, dtype=np.int32)
        self.all_vaccinated = np.zeros(nr_ages * 2, dtype=np.int32)
        self.immunized = np.zeros(nr_ages * 2, dtype=np.int32)
        self.all_immunized = np.zeros(nr_ages * 2, dtype=np.int32)
    ...
    cdef _create_agents(self, age_counts):
        cdef int idx, person_idx, age, sex
        cdef Person * p
        ...
        self.age_start = np.empty(self.nr_ages * 2, dtype=np.int32)
    ...
    cdef void vaccinate(self, Person * person) nogil:
        ...
        cdef int sex = person.sex
        self.vaccinated[age + 101 * sex] += 1
        self.all_vaccinated[age + 101 * sex] += 1

    cdef void immunize(self, Person * person) nogil:
        cdef int age = person.age
        cdef int sex = person.sex
        self.susceptible[age + 101 * sex] -= 1
        self.immunized[age + 101 * sex] += 1
        self.all_immunized[age + 101 * sex] += 1

    cdef void infect(self, Person * person) nogil:
        cdef int age = person.age
        cdef int sex = person.sex
        self.susceptible[age + 101 * sex] -= 1
        self.infected[age + 101 * sex] += 1
        self.all_infected[age + 101 * sex] += 1

    cdef void recover(self, Person * person) nogil:
        cdef int age = person.age
        cdef int sex = person.sex
        self.infected[age + 101 * sex] -= 1
        self.recovered[age + 101 * sex] += 1
        if person.was_detected:
            self.detected[age + 101 * sex] -= 1

    cdef void detect(self, Person * person) nogil:
        cdef int age = person.age
        cdef int sex = person.sex
        self.detected[age + 101 * sex] += 1
        self.all_detected[age + 101 * sex] += 1

    cdef void hospitalize(self, Person * person) nogil:
        cdef int age = person.age
        cdef int sex = person.sex
        self.hospitalized[person.age + 101 * sex] += 1

    cdef void transfer_to_icu(self, Person * person) nogil:
        cdef int age = person.age
        cdef int sex = person.sex
        self.in_icu[person.age + 101 * sex] += 1
...
cdef class Context:
    ...
    # NEW: Vaccination
    cdef int total_vaccinated, total_immunized
    ...
    def add_intervention(self, day, name, value):
        ...
        # print(f'day: {day}, adding intervention: {name}, value: {value}')
        if value is None:
            value = 0
    ...
    def generate_state(self):
        ...
        s = dict(
            ...
            tests_run_per_day=hc.tests_run_per_day,
            mobility_limitation=1 - p.population_mobility_factor,

            # NEW: Vaccination
            vaccinated=p.vaccinated,
            all_vaccinated=p.all_vaccinated,
            daily_vaccination_dose1 = hc.daily_vaccination_dose1,
            daily_vaccination_dose2 = hc.daily_vaccination_dose2,
            all_vaccination_dose1 = hc.all_vaccination_dose1,
            all_vaccination_dose2 = hc.all_vaccination_dose2,
            immunized = p.immunized,
            all_immunized = p.all_immunized,
        )
        return s
    ...
    # From intervention -> introduces random infectees
    ...
    # From intervention -> introduces random recovered (to simulate from a given point)
    def introduce_recovered_people(self, count):
        cdef int idx
        cdef Person * person

        for i in range(count):
            person = self.pop.get_random_person(self)
            person_recover_introduced(person, self)
    ...
    def apply_intervention(self, name, value):
        ...
        elif name == 'import-recovered':
            # Introduce recovered people
            count = value
            self.introduce_recovered_people(count)
        ...
        elif name == 'set-test-avg-delay':
            self.hc.test_delay_avg_days = value
    ...
    cdef inline void _process_person(self, Person *person) nogil:
        ...
        # NEW: Vaccination
        if person.vaccine_doses and person.day_of_immunization == self.day:
            person.is_immunized = 1
            if person.state == PersonState.SUSCEPTIBLE:
                person.state = PersonState.IMMUNIZED
            self.pop.immunize(person)
